...
public class SortMerge extends Iterator implements GlobalConst
{
  ...
  public SortMerge(AttrType    in1[],
	...
	throw new MapUtilsException (e, "Exception is caught by SortMerge.java");
      ...
      if    (!in1_sorted){
	try {
	  p_i1 = new Sort(in1, s1_sizes, am1, join_col_in1,
			  order, sortFld1Len, amt_of_mem / 2, true);
	}catch(Exception e){
      ...
      if (! in2_sorted){
	try {
	  p_i2 = new Sort(in2, s2_sizes, am2, join_col_in2,
			   order, sortFld2Len, amt_of_mem / 2, true);
	}catch(Exception e){
	  throw new SortException (e, "Sort failed");
	}
      }
  ...
  public Map get_next()
      ...
      while (true)
	{
	  ...
	  if (PredEval.Eval(OutputFilter, TempMap1, TempMap2))
	    {
