...
public class SortMerge extends MapIterator implements GlobalConst {
    ...
    public SortMerge(AttrType in1[],
        ...
        if (!in1_sorted) {
            try {
                BigTable.mapInsertOrder = false;
                BigTable.orderType = 1;
                p_i1 = new MapSort(BigTable.BIGT_ATTR_TYPES, BigTable.BIGT_STR_SIZES, am1, join_col_in1, order, amt_of_mem / 2, sortFld1Len, false);
            } catch (Exception e) {
        ...
        if (!in2_sorted) {
            try {
                BigTable.mapInsertOrder = false;
                BigTable.orderType = 1;
                p_i2 = new MapSort(BigTable.BIGT_ATTR_TYPES, BigTable.BIGT_STR_SIZES, am2, join_col_in2, order, amt_of_mem/2, sortFld2Len, false);
            } catch (Exception e) {
                throw new SortException(e, "Sort failed");
            }
        }
