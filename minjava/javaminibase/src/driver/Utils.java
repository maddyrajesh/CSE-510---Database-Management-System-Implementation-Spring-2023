        ...
        try {
            ...
            while ((inputStr = br.readLine()) != null) {
                ...
                if (input[1].length() > BigTable.BIGT_STR_SIZES[1]) {
                    input[1] = input[1].substring(0, BigTable.BIGT_STR_SIZES[1]);
                }
                if (input[2].length() > BigTable.BIGT_STR_SIZES[2]) {
                    input[2] = input[2].substring(0, BigTable.BIGT_STR_SIZES[2]);
                }
            ...
            try {
                fscan = new FileScan(tableName + "tempfile", BigTable.BIGT_ATTR_TYPES,
                        BigTable.BIGT_STR_SIZES, (short) 4, 4, projlist, null);
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            try {
                ...
                sort = new MapSort(BigTable.BIGT_ATTR_TYPES, BigTable.BIGT_STR_SIZES, fscan, 1, new MapOrder(MapOrder.Ascending), 20, 25, type);
            } catch (Exception e) {
                e.printStackTrace();
            }
            ...
            FileWriter fileWriter = new FileWriter("resultsash");
            int count = 1;
            while (m != null) {
//                if ((!oldRowLabel.equals(m.getRowLabel()) && oldColumnLabel.equals(m.getColumnLabel())) || ((oldRowLabel.equals(m.getRowLabel()) && !oldColumnLabel.equals(m.getColumnLabel()))) || ((oldRowLabel.equals(m.getRowLabel()) && oldColumnLabel.equals(m.getColumnLabel())))) {
                ...
                insertMap.setHeader(BigTable.BIGT_ATTR_TYPES, BigTable.BIGT_STR_SIZES);
                insertMap.copyMap(m);
...
//            System.out.println(evictingQueue.size());
            ...
            for (int i = 0; i < 5; i++) {
                ...
                System.out.println("===========================");
                Scan mapScan = bigTable.heapfiles[i].openScan();
                MID mid = new MID();
    ...
    /*public static void rowSort(String inTableName, String outTableName, String columnName, int NUMBUF) throws Exception {
        ...
        rowSort rowSort = new BigT.rowSort(inTableName, columnName, NUMBUF);
...
//        new SystemDefs(dbPath, 0, NUMBUF, "Clock");
    ...
    public static void addTableToInventory(String bigTable) throws Exception {
        try {
            ...
            tableInfo.setHeader(BigTable.BIGT_ATTR_TYPES, BigTable.BIGT_STR_SIZES);
    ...
    public static List<String> getAllTablesInventory() throws InvalidTupleSizeException, IOException, HFDiskMgrException, HFBufMgrException, HFException, InvalidMapSizeException {
        ...
        Scan scan = bigTInventory.openScan();
        MID mid = new MID();
    ...
    public static void insertMap(String bigtName, int indextype, String rowLabel, String columnLabel, String ValueInfo, int timeStampVal, int NUMBUF) throws Exception {
        File file = new File(Utils.getDBPath());
        if(file.exists())
            new SystemDefs(Utils.getDBPath(), 0, NUMBUF, "Clock");
        else {
            new SystemDefs(Utils.getDBPath(), Utils.NUM_PAGES, NUMBUF, "Clock");
        }
        ...
        try{
            ...
            Map mapObj = new Map();
            mapObj.setHeader(BigTable.BIGT_ATTR_TYPES, BigTable.BIGT_STR_SIZES);
    ...
    public static void rowJoin(String tableName1, String tableName2, String outputTableName, String colFilter, String joinType, Integer NUMBUF) throws Exception {
        ...
        Stream leftstream = new bigt(tableName1, true).openStream(1, "*", colFilter, "*");
        rj = new rowJoin(20, leftstream, tableName2, colFilter, outputTableName, tableName1);

        System.out.println("Query results => ");
        Utils.query(outputTableName, new Integer(1),"*", "*", "*", NUMBUF);
...
//        new SystemDefs(dbPath, 0, NUMBUF, "Clock");
        System.out.println("Row Sort results=>");
        bigt result = new bigt( outputTableName, false);
        Scan mapScan = result.heapfiles[0].openScan();
        MID mid = new MID();
        Map map1 = mapScan.getNext(mid);
        while (map1 != null) {
            map1.print();
...
//rowsort mapInsertTest rs Moose 50   query rs 1 1 * * * 50
}