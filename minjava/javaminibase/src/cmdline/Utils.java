    ...
    public static void batchInsert(String dataFile, String tableName, int type, int numBufs) throws Exception {
        ...
        try {
            ...
            try {
                ...
                sort = new MapSort(MiniTable.BIGT_ATTR_TYPES, MiniTable.BIGT_STR_SIZES, fscan, 1, new MapOrder(MapOrder.Ascending), 20, 25, false);
            } catch (Exception e) {
                e.printStackTrace();
            }
...
//        new SystemDefs(dbPath, 0, NUMBUF, "Clock");
        ...
        MapScan mapScan = result.heapfile.openMapScan();
        MID mid = new MID();
...
/*    public static void getCounts(Integer numBufs) throws Exception {
    ...
    public static void insertMap(String bigtName, int indextype, String rowLabel, String columnLabel, String ValueInfo, int timeStampVal, int NUMBUF) throws Exception {
        ...
        pcounter.initialize();
        try{
            bigt inBigTName = new bigt(bigtName, indextype);
            Map mapObj = new Map();
            ...
            inBigTName.insertMap(mapObj.getMapByteArray());
            mapObj.print();
            inBigTName.close();
